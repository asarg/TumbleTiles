...
emptyBoardFile = "ReconfigurationEmpty.xml"
...
def setTileToPosition(tile, position):
	tile.x = int(position[:2])
	tile.y = int(position[2:4])

def recurseStartingCoordinates(startingPosition, tile, path):
	global board
	...
	if (path == "1" and checkFor == "valid") or (path == "2" and checkFor == "invalid"):
		startingPositions.add(blueStartN + startingPosition)
		startingPositions.add(blueStartE + startingPosition)
	else:
		startingPositions.add(blueStartS + startingPosition)
		startingPositions.add(blueStartW + startingPosition)

	setTileToPosition(tile, startingPosition)

	if True:
		board.Tumble("N")
		newConfig = getTileCoords(tile)
		if not newConfig in redStartPositions:
			recurseStartingCoordinates(newConfig, tile, path)
